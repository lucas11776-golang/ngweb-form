...
export class NgwebFormInputErrorMessageDirective implements OnDestroy, OnChanges {
  @Input() set ngwebFormInputErrorMessage(input: [FormGroup, string, boolean]) {
    this.element = <HTMLElement>this.el.nativeElement;
    this.sub.sink = this.listener(input[0], input[1], input[2]);
  }
  @Input("formInputErrorMessageClass") ngwebFormInputErrorMessageClass: string = "";
  @Input("formInputErrorMessageExternal") ngwebFormInputErrorMessageExternal: string|null|undefined = "";
